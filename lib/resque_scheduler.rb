...
module ResqueScheduler
  ...
  def enqueue_at_with_queue(queue, timestamp, klass, *args)
    Rails.logger.debug "AAAAA: invoking enqueue_at_with_queue"

    Rails.logger.debug "AAAAA: running before schedule hooks"
    return false unless Plugin.run_before_schedule_hooks(klass, *args)

    Rails.logger.debug "AAAAA: checking if resque is inline or if the timestamp is before now"
    if Resque.inline? || timestamp.to_i < Time.now.to_i

      Rails.logger.debug "AAAAA: resque is inline or the timestamp is before now"
      ...
      if klass.respond_to?(:scheduled)
        Rails.logger.debug "AAAAA: klass responds to scheduled"
        klass.scheduled(queue, klass.to_s(), *args)
      else
        Rails.logger.debug "AAAAA: klass does not respond to scheduled"
        Resque::Job.create(queue, klass, *args)
      end
    else
      Rails.logger.debug "AAAAA: invoking delayed_push: #{timestamp}"
      delayed_push(timestamp, job_to_hash_with_queue(queue, klass, args))
    end

    Rails.logger.debug "AAAAA: running after schedule hooks"
